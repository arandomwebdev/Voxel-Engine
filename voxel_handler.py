...
class VoxelHandler:
    def __init__(self, world):
        ...
        self.voxel_types = [GRASS, DIRT, STONE, SNOW, WOOD, SAND, LEAVES]
        self.voxel_type_names = ['GRASS', 'DIRT', 'STONE', 'SNOW', 'WOOD', 'SAND', 'LEAVES']
        self.selected_block_index = 0
        self.new_voxel_id = self.voxel_types[self.selected_block_index]

    def select_block(self, index):
        # Select a specific block by index (0-6)
        if 0 <= index < len(self.voxel_types):
            self.selected_block_index = index
            self.new_voxel_id = self.voxel_types[self.selected_block_index]
            print(f"{self.voxel_type_names[self.selected_block_index]}")
    ...
    def is_solid(self, world_pos):
        # Force integer voxel coordinates
        world_pos = glm.ivec3(world_pos)

        voxel_id, voxel_index, voxel_local_pos, chunk = self.get_voxel_id(world_pos)
